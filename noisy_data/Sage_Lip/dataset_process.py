...
def load_data(dataset_str):
    ...
    for i in range(len(names)):
        with open("../data_file/ind.{}.{}".format(dataset_str, names[i]), 'rb') as f:
    ...
    test_idx_reorder = parse_index_file("../data_file/ind.{}.test.index".format(dataset_str))
    ...
    labels = torch.LongTensor(np.where(labels)[1])    #将one-hot向量转化为矩阵

    if dataset_str == 'cora':   # 进行数据集的划分
        idx_train = range(300)
        idx_val = range(300, 800)
        idx_test = range(1708, 2707)
        # idx_train = range(140)
...
def normalize_adj(adj):      #对于graphsage而言，这一步用来对于邻居信息进行mean操作
